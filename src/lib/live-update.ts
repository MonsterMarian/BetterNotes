...
const URL_KEY = "betternotes:ota:url";
...
export const DEFAULT_UPDATE_URL =
  "https://api.github.com/repos/MonsterMarian/BetterNotes/contents/ota/latest.json?ref=main";

const FALLBACK_UPDATE_URL =
  "https://raw.githubusercontent.com/MonsterMarian/BetterNotes/main/ota/latest.json";
...
export function markBootSucceeded(): void {
  write(BOOTING_KEY, null);
  (window as unknown as { __bnBooted?: boolean }).__bnBooted = true;
}
...
export async function applyPendingUpdate(): Promise<ApplyResult> {
  ...
  try {
    ...
    // Není co nasazovat - ale ještě se musí ověřit, odkud se vlastně servíruje.
    //
    // Capacitor si tuhle cestu ukládá NATIVNĚ (`persistServerBasePath`), ne
    // v localStorage. Když se tedy localStorage vyprázdní (vymazání dat appky,
    // návrat na verzi z APK, ruční úklid) a nativní cesta zůstane viset na
    // starém balíku, appka se z něj spouští dál - i po přeinstalaci APK.
    // Zvenčí to vypadá, že se nová verze vůbec nenainstalovala.
    if (!target) {
